...
# CONFIGURATION
# -----------------------------
AUDIO_DIR = "/scratch/ym4c23/Mic/Step3_/result/miniaudio"
CSV_PATH = "/scratch/ym4c23/Mic/Step3_/result/miniCSV/angles_with_mics.csv"
BATCH_SIZE = 4
EPOCHS = 60
LEARNING_RATE = 1e-3
PATIENCE = 10
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in dataloader:
            # ✅ 修改 1：增加 mic_pos 输入
            waveform, mic_pos, angle = batch
            waveform = waveform.to(device)
            mic_pos = mic_pos.to(device)
            angle = angle.to(device)

            waveform = waveform.permute(0, 2, 1)              # (B, N, 4)
            mic_pos = mic_pos.view(mic_pos.size(0), -1)       # (B, 12)

            # ✅ 修改 2：传入 mic_pos 到模型
            angle_pred = model(waveform, mic_pos)             # (B,)
